...
static HeightMapper::Config loadConfig(const ros::NodeHandle &nh) {
  ...
  nh.param<double>("map/min_height", cfg.min_height, -0.2);
  nh.param<double>("map/max_height", cfg.max_height, 1.5);
...
static GlobalMapper::Config loadConfig(const ros::NodeHandle &nh) {
  ...
  nh.param<std::string>("map_save_dir", cfg.map_save_dir, "/home/tre0430/map");
